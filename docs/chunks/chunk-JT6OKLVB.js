...
var QrScanner = class {
  ...
  async start() {
    ...
    const framePromise = this.#video.readyState >= 2 ? Promise.resolve() : new Promise((resolve) => {
  ...
  #tickJsQr() {
    ...
    if (v.readyState >= v.HAVE_ENOUGH_DATA && v.videoWidth && v.videoHeight) {
      ...
      const result = (0, import_jsqr.default)(img.data, img.width, img.height);
