...
function createProcessor ({
  ...
  return async function processOuterEvent (outer) {
    ...
    try {
      ...
      if (err?.message === 'INVALID_SIGNED_INNER_EVENT' && groupKey) {
        ignoredGroups.add(groupKey)
        receivedChunks.removeGroup(groupKey)
      }
      onError?.(err)
